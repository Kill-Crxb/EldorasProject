...
public class DamageModule : MonoBehaviour, IBrainModule
{
    ...
    {
        float baseDamage = attackData.weaponDamageMultiplier * 10f; // Base from weapon
        ...
        // Get damage type from attack data or use physical
        ...
        if (debugDamage)
        {
            Debug.Log($"[DamageModule] {gameObject.name} calculated damage: {baseDamage:F1} (crit: {isCritical})");
        }
