...
public class PlayerItemsModule : MonoBehaviour, IPlayerModule, ISaveable, IInventoryProvider, IEquipmentProvider
{
    ...
    {
        return inventoryItems.Any(i => i != null && i.definitionId == itemId);
    }

    int IInventoryProvider.GetItemCount(string itemId)
    {
        return inventoryItems.Count(i => i != null && i.definitionId == itemId);
    }
    ...
    bool IInventoryProvider.AddItem(string itemId, int quantity)
    {
        ...
        {
            var instance = new ItemInstance(itemId);
            if (!AddItem(instance))
                return false;
        }
        return true;
    ...
    bool IInventoryProvider.RemoveItem(string itemId, int quantity)
    {
        ...
        {
            if (inventoryItems[i] != null && inventoryItems[i].definitionId == itemId)
            {
    ...
    string IEquipmentProvider.GetEquippedItem(EquipmentSlotType slot)
    {
        var equipSlot = ConvertSlotType(slot);
        var item = GetEquippedItem(equipSlot);
        return item?.definitionId;
    }

    bool IEquipmentProvider.EquipItem(string itemId, EquipmentSlotType slot)
    {
        var instance = inventoryItems.FirstOrDefault(i => i != null && i.definitionId == itemId);
        if (instance == null) return false;

        var equipSlot = ConvertSlotType(slot);
        return EquipItem(instance, equipSlot);
    }

    bool IEquipmentProvider.UnequipItem(EquipmentSlotType slot)
    {
        var equipSlot = ConvertSlotType(slot);
        return UnequipItem(equipSlot);
    }

    bool IEquipmentProvider.IsSlotOccupied(EquipmentSlotType slot)
    {
        var equipSlot = ConvertSlotType(slot);
        return GetEquippedItem(equipSlot) != null;
    }

    event Action<EquipmentSlotType, string> IEquipmentProvider.OnItemEquipped
    {
        add { /* Wrap existing event */ }
        remove { /* Wrap existing event */ }
    }

    event Action<EquipmentSlotType> IEquipmentProvider.OnItemUnequipped
    {
        add { /* Wrap existing event */ }
        remove { /* Wrap existing event */ }
    }

    private EquipmentSlot ConvertSlotType(EquipmentSlotType slotType)
    {
        return slotType switch
        {
            EquipmentSlotType.Head => EquipmentSlot.Helmet,
            EquipmentSlotType.Chest => EquipmentSlot.Armor,
            EquipmentSlotType.Legs => EquipmentSlot.Belt,
            EquipmentSlotType.Feet => EquipmentSlot.Boots,
            EquipmentSlotType.Hands => EquipmentSlot.Gloves,
            EquipmentSlotType.MainHand => EquipmentSlot.Weapon1,
            EquipmentSlotType.OffHand => EquipmentSlot.Weapon2,
            EquipmentSlotType.Accessory1 => EquipmentSlot.Pouch,
            EquipmentSlotType.Accessory2 => EquipmentSlot.Rig,
            _ => EquipmentSlot.Weapon1
        };
    }
