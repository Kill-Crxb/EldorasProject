...
/// <summary>
/// AnimationProvider - Provides animation interface to other modules
...
/// </summary>
public class AnimationProvider : MonoBehaviour, IBrainModule, IAnimationProvider
{
    [Header("Debug")]
    ...
    public void Initialize(ControllerBrain controllerBrain)
    {
        ...
        {
            Debug.LogWarning($"[AnimationProvider] No Animator found yet on {gameObject.name} - will use when available");
            return;
        }

        if (debugAnimations)
        {
            Debug.Log($"[AnimationProvider] Initialized with Animator on {brain.EntityAnimator.gameObject.name}");
            Debug.Log($"[AnimationProvider] Animator has {brain.EntityAnimator.parameterCount} parameters");
        }
    }
    ...
    public void SetTrigger(string parameterName)
    {
        ...
        if (debugAnimations)
            Debug.Log($"[AnimationProvider] SetTrigger: {parameterName}");
    ...
    public void ResetTrigger(string parameterName)
    {
        ...
        if (debugAnimations)
            Debug.Log($"[AnimationProvider] ResetTrigger: {parameterName}");
    ...
    public void SetBool(string parameterName, bool value)
    {
        ...
        if (debugAnimations)
        {
            ...
            {
                Debug.Log($"[AnimationProvider] SetBool: {parameterName} = {value}");
    ...
    public void SetFloat(string parameterName, float value)
    {
        ...
        if (debugAnimations)
        {
            ...
            if (shouldLog)
            {
                Debug.Log($"[AnimationProvider] SetFloat: {parameterName} = {value:F3}");
                parameterCache[parameterName] = value;
            }
        }
    ...
    public void SetInteger(string parameterName, int value)
    {
        ...
        if (debugAnimations)
        {
            ...
            {
                Debug.Log($"[AnimationProvider] SetInteger: {parameterName} = {value}");
                parameterCache[parameterName] = value;
            }
        }
    ...
    public void Play(string stateName, int layerIndex = 0)
    {
        ...
        if (debugAnimations)
            Debug.Log($"[AnimationProvider] Play: {stateName} (Layer {layerIndex})");
    ...
    public void CrossFade(string stateName, float transitionDuration, int layerIndex = 0)
    {
        ...
        if (debugAnimations)
            Debug.Log($"[AnimationProvider] CrossFade: {stateName} (Duration: {transitionDuration:F2}, Layer {layerIndex})");
    ...
    public void SetLayerWeight(int layerIndex, float weight)
    {
        ...
        if (debugAnimations)
            Debug.Log($"[AnimationProvider] SetLayerWeight: Layer {layerIndex} = {weight:F2}");
    }
    ...
    public void DebugLogAllParameters()
    {
        if (animator == null)
        {
            Debug.LogWarning("[AnimationProvider] No animator assigned!");
            return;
        }
