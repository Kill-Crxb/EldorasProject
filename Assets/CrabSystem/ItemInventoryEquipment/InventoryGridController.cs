...
public class InventoryGridController : MonoBehaviour
{
    ...
    /// <summary>
    /// Manually set the ItemSystem (used by ContainerWindow for non-player inventories)
    /// </summary>
    public void SetItemSystem(ItemSystem system)
    {
        // Ensure we're initialized first
        if (overlays == null || icons == null)
        {
            Initialize();
        }

        // Unsubscribe from old system if exists
        if (itemSystem != null)
        {
            itemSystem.OnInventoryChanged -= RefreshAllItems;
        }
        ...
        {
            itemSystem.OnInventoryChanged += RefreshAllItems;
            RefreshAllItems(); // Immediate refresh with new data

            if (showDebugInfo)
                Debug.Log($"Manually set ItemSystem: {system.name}");
        }
    }
    ...
    private void ConnectToPlayerData()
    {
        ...
        if (brain != null)
        {
            itemSystem = brain.GetModule<ItemSystem>();
            if (itemSystem != null)
            {
                itemSystem.OnInventoryChanged += RefreshAllItems;

                if (showDebugInfo)
                    Debug.Log("Connected to ItemSystem");
            }
            else
            {
                Debug.LogWarning("ItemSystem not found on ControllerBrain!");
            }
        }
        else
        {
    ...
    private void RefreshAllItems()
    {
        ...
        {
            if (showDebugInfo)
                Debug.Log("Cannot refresh items - ItemSystem not connected");
            return;
        }

        var items = itemSystem.GetAllInventoryItems();
        int itemCount = 0;
        int overlayCount = 0;
    ...
    public void OnItemDragEnd(Vector2 screenPos, string itemId, GridArea itemArea)
    {
        ...
        else
        {
            ...
            if (showDebugInfo)
            {
                var item = itemSystem?.GetItemInstance(itemId);
                if (item != null)
    ...
    {
        if (itemSystem == null) return null;
        ...
        foreach (var item in itemSystem.GetAllInventoryItems())
        {
    ...
    public void PrintGridState()
    {
        ...
        if (itemSystem != null)
            Debug.Log($"PlayerItems: {itemSystem.GetAllInventoryItems().Length} items");
