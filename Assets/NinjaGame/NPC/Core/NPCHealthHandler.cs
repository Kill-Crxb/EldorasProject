...
public class NPCHealthHandler : MonoBehaviour, INPCHandler
{
    ...
    public void LoadHandlerData(string json)
    {
        ...
        if (isDead)
        {
            HandleDeath();  // FIXED: Removed parameter
        }
    }
    ...
    private void HandleDamageReceived(CombatDamagePacket packet)
    {
        ...
        float damage = packet.finalDamage;  // FIXED: Use packet.finalDamage
        ...
        if (debugLogs)
        {
            string attackerName = packet.attackerId ?? "Unknown";  // FIXED: Use packet
            Debug.Log($"[NPCHealthHandler] {parentNPC.NPCName} took {damage:F1} damage from {attackerName}\n" +  // FIXED: Use damage variable
    ...
    /// </summary>
    private void HandleDeath()  // FIXED: Removed parameter
    {
        ...
        if (debugLogs)
        {
            Debug.Log($"<color=red>[NPCHealthHandler] ☠ {parentNPC.NPCName} has DIED!</color>");  // FIXED: Removed killingBlow reference
        }
