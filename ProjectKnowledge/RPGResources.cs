...
public class RPGResources : MonoBehaviour, IPlayerModule, IHealthProvider, IResourceProvider
{
    ...
    [Header("Resource Masking (Performance Optimization)")]
    [Tooltip("Mask out resources this entity doesn't use. Bears don't use mana, for example.")]
    [SerializeField] private ResourceMaskFlags enabledResources = ResourceMaskFlags.AllResources;
    ...
    /// <summary>
    /// Check if a specific resource should be updated based on the mask.
    /// </summary>
    private bool IsResourceEnabled(ResourceMaskFlags flag)
    {
        return (enabledResources & flag) != 0;
    }
    ...
    // Events (public - can be subscribed to directly)
    ...
    // FIXED: Interface events that need to actually fire
    private event System.Action<float> healthProviderOnHealthChanged;
    private event System.Action healthProviderOnDeath;
    private event System.Action<ResourceType, float> resourceProviderOnResourceChanged;
    ...
    public void Initialize(ControllerBrain brain)
    {
        ...
        if (debugStats && showMaskingInfo)
        {
            var enabled = new System.Collections.Generic.List<string>();
            if (IsResourceEnabled(ResourceMaskFlags.Health)) enabled.Add("Health");
            if (IsResourceEnabled(ResourceMaskFlags.Mana)) enabled.Add("Mana");
            if (IsResourceEnabled(ResourceMaskFlags.Stamina)) enabled.Add("Stamina");
            Debug.Log($"[RPGResources] Resource Masking - Active: {string.Join(", ", enabled)} | Mask: {enabledResources}");
        }
    }
    ...
    void HandleAutoRegeneration()
    {
        ...
        // Health regeneration (slower) - masked
        if (IsResourceEnabled(ResourceMaskFlags.Health) && currentHealth < maxHealth && time - lastHealthRegen >= 1f / healthRegenRate)
        {
        ...
        // Mana regeneration (medium) - masked
        if (IsResourceEnabled(ResourceMaskFlags.Mana) && currentMana < maxMana && time - lastManaRegen >= 1f / manaRegenRate)
        {
        ...
        // Stamina regeneration (faster) - masked
        if (IsResourceEnabled(ResourceMaskFlags.Stamina) && currentStamina < maxStamina && time - lastStaminaRegen >= 1f / staminaRegenRate)
        {
    ...
    public void ModifyHealth(float amount)
    {
        ...
        if (Mathf.Abs(oldHealth - currentHealth) > 0.001f)
        {
            ...
            healthProviderOnHealthChanged?.Invoke(currentHealth);
            resourceProviderOnResourceChanged?.Invoke(ResourceType.Health, currentHealth);

            // FIXED: Check for death and fire OnDeath event
            bool isDead = currentHealth <= 0f;
            if (wasAlive && isDead)
            {
                healthProviderOnDeath?.Invoke();
                wasDead = true;
            }
            // Track resurrection
            else if (!wasAlive && !isDead)
            {
                wasDead = false;
            }
        }
    ...
    public void ModifyMana(float amount)
    {
        ...
        if (Mathf.Abs(oldMana - currentMana) > 0.001f)
        {
            ...
            resourceProviderOnResourceChanged?.Invoke(ResourceType.Mana, currentMana);
        }
    }
    ...
    public void ModifyStamina(float amount)
    {
        ...
        if (Mathf.Abs(oldStamina - currentStamina) > 0.001f)
        {
            ...
            resourceProviderOnResourceChanged?.Invoke(ResourceType.Stamina, currentStamina);
        }
    }
    ...
    float IHealthProvider.GetCurrentHealth() => currentHealth;
    float IHealthProvider.GetMaxHealth() => maxHealth;
    float IHealthProvider.GetHealthPercentage() => HealthPercentage * 100f;
    bool IHealthProvider.IsAlive() => !IsDead();

    void IHealthProvider.ApplyDamage(float damage) => ModifyHealth(-damage);
    void IHealthProvider.ApplyHealing(float healing) => ModifyHealth(healing);
    void IHealthProvider.SetHealth(float value) => ModifyHealth(value - currentHealth);
    void IHealthProvider.SetHealthToMax() => SetHealthToMax();

    // FIXED: Actually implement the events properly
    event System.Action<float> IHealthProvider.OnHealthChanged
    {
        add { healthProviderOnHealthChanged += value; }
        remove { healthProviderOnHealthChanged -= value; }
    }

    event System.Action IHealthProvider.OnDeath
    {
        add { healthProviderOnDeath += value; }
        remove { healthProviderOnDeath -= value; }
    }
    ...
    float IResourceProvider.GetResource(ResourceType type) => type switch
    {
    ...
    float IResourceProvider.GetMaxResource(ResourceType type) => type switch
    {
    ...
    float IResourceProvider.GetResourcePercentage(ResourceType type) => type switch
    {
        ResourceType.Health => HealthPercentage * 100f,
        ResourceType.Mana => ManaPercentage * 100f,
        ResourceType.Stamina => StaminaPercentage * 100f,
        _ => 0f
    };

    bool IResourceProvider.HasResource(ResourceType type, float amount) => type switch
    {
        ResourceType.Health => HasHealth(amount),
        ResourceType.Mana => HasMana(amount),
        ResourceType.Stamina => HasStamina(amount),
        _ => false
    };

    bool IResourceProvider.ConsumeResource(ResourceType type, float amount)
    {
        if (!((IResourceProvider)this).HasResource(type, amount)) return false;
        switch (type)
        {
            case ResourceType.Health: ModifyHealth(-amount); break;
            case ResourceType.Mana: ModifyMana(-amount); break;
            case ResourceType.Stamina: ModifyStamina(-amount); break;
            default: return false;
        }
        return true;
    }

    void IResourceProvider.RestoreResource(ResourceType type, float amount)
    {
        switch (type)
        {
            case ResourceType.Health: ModifyHealth(amount); break;
    ...
    void IResourceProvider.SetResourceToMax(ResourceType type)
    {
        switch (type)
        {
            case ResourceType.Health: SetHealthToMax(); break;
            case ResourceType.Mana: SetManaToMax(); break;
            case ResourceType.Stamina: SetStaminaToMax(); break;
        }
    }

    // FIXED: Actually implement the event
    event System.Action<ResourceType, float> IResourceProvider.OnResourceChanged
    {
        add { resourceProviderOnResourceChanged += value; }
        remove { resourceProviderOnResourceChanged -= value; }
    }
