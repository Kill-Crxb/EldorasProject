...
public class SimpleAttackHitbox : MonoBehaviour
{
    ...
    // REPLACE the OnTriggerEnter method in SimpleAttackHitbox.cs (around line 120) with this:
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        // MODERN PATTERN: Try Brain + DamageSystem first (for Player/NPCs)
        ...
        if (targetBrain != null)
        {
            var targetDamageSystem = targetBrain.GetModule<DamageSystem>();
            if (targetDamageSystem != null)
            {
                ...
                // Apply damage via DamageSystem (modern)
                targetDamageSystem.TakeDamage(packet);

                if (debugHitbox)
                {
                    Debug.Log($"[SimpleAttackHitbox] Hit {other.name} (Brain) for {packet.finalDamage:F1} damage!");
                }
                return;
            }
        }

        // LEGACY PATTERN: Fall back to IDamageable (for simple destructibles)
        ...
        if (damageable != null)
        {
            ...
            // Apply damage via legacy interface
            damageable.TakeDamage(packet.finalDamage);

            if (debugHitbox)
            {
                Debug.Log($"[SimpleAttackHitbox] Hit {other.name} (Legacy) for {packet.finalDamage:F1} damage!");
            }
        }
        else if (debugHitbox)
        {
    ...
    // KEY CHANGES:
    // 1. Check for ControllerBrain first (MODERN pattern)
    // 2. Get DamageSystem module from brain
    // 3. Call targetDamageSystem.TakeDamage(packet) - passes full packet, not just damage value
    // 4. Fall back to IDamageable for legacy destructibles (crates, barrels)
    void OnDrawGizmos()
    {
