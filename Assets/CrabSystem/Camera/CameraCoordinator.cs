...
public class CameraCoordinator : MonoBehaviour, IBrainModule, ICameraProvider
{
    ...
    // ICameraProvider implementation - returns actual Camera transform
    public Transform CameraTransform
    {
        get
        {
            if (cachedCameraTransform == null && cameraModule != null)
            {
                // Try to find Camera component on same GameObject
                Camera cam = cameraModule.GetComponent<Camera>();
                if (cam != null)
                {
                ...
                else
                {
                    ...
                    cam = cameraModule.GetComponentInChildren<Camera>();
                    if (cam != null)
                    {
                        cachedCameraTransform = cam.transform;
                    }
                    ...
                    {
                        // Fallback: use the camera module's transform
                        cachedCameraTransform = cameraModule.transform;
                        Debug.LogWarning($"[CameraCoordinator] No Camera component found on {cameraModule.name}, using GameObject transform as fallback");
                    }
                }
            }
